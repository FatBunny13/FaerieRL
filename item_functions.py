...
def become_graceful(*args, **kwargs):
    entity = args[0]

    results = []

    results.append({'consumed': False, 'message': Message('You suddenly feel graceful!', libtcod.yellow)})
    entity.fighter.status_effects.grace=True


    return results

def become_accurate(*args, **kwargs):
    entity = args[0]

    results = []

    results.append({'consumed': False, 'message': Message('You suddenly feel graceful!', libtcod.yellow)})
    entity.fighter.status_effects.accurate=True
